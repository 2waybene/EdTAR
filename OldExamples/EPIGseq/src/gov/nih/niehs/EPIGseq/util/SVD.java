      ...
      while (p > 0) {
         ...
         if (k == p-2) {
            ...
            for (ks = p-1; ks >= k; ks--) {
               ...
               float t = (ks != p ? (float)Math.abs(e[ks]) : 0.f) +
                          (ks != k+1 ? (float)Math.abs(e[ks-1]) : 0.f);
 ...
 * ------------------------ */
   ...
   */
   ...
   float [] w;
   float [][] v;
   public float [] getSingularValue() {
     return w;
   }
   public float [][] getMatrixV() {
     return v;
   }
   public void svdcmp(float [][] a) {
     ...
     v = new float[n][n];
      ...
      for (i=0;i<n;i++) {
        ...
        if(i<m) {
          ...
          if (scale!=0) {
            ...
            a[i][i]=f-g;
            for (j=l;j<n;j++) {
              for (s=0,k=i;k<m;k++) s+=a[k][i]*a[k][i];
              f=s/h;
        ...
        if(i<m && i != n-1) {
          ...
          if (scale!=0) {
            ...
            for (j=0;j<m;j++) {
              for (s=0,k=0;k<n;k++)s+=a[j][k]*a[i][k];
      ...
      for (i=IMIN(m,n);i>=0;i--) {
        ...
        if (g!=0){
          g=1/g;
          for(j=l;j<n;j++) {
            for(s=0,k=l;k<m;k++) s+=a[k][i]*a[k][i];
            f=(s/a[i][i])*g;
            for (k=i;k<m;k++)a[k][j]+=f*a[k][i];
          }
   ...
   public float pythag(float a, float b) {
     ...
     if (absa > absb) {
       float r = absb/absa;
       return absa*(float)Math.sqrt(1+r*r);
     }
     else {
       if (absb == 0.0f ) return 0.f;
       float r = absa/absb;
       return absb*(float)Math.sqrt(1+r*r);
     }
   }
